...
import {
    ...
    Loader2, Link, Tag, ExternalLink, RefreshCw,
    Github, ChevronLeft, ChevronRight, FileText
} from "lucide-react";
...
export default function BookmarksPage() {
    ...
    // Fetch topics — ambil page 1 limit 100 lalu paralel sisanya
    ...
    const fetchBookmarks = useCallback(async (isPageChange = false) => {
        // Ganti page/topic/search → pakai refreshing (spinner kecil), bukan skeleton penuh
    ...
    // Mount pertama kali → full skeleton
    useEffect(() => { fetchBookmarks(false); }, []);

    // Ganti page → spinner kecil saja
    useEffect(() => {
        if (!initialLoading) fetchBookmarks(true);
    }, [page]);

    // Ganti search/topic → reset page 1 + spinner
    useEffect(() => {
        if (!initialLoading) {
            setPage(1);
            fetchBookmarks(true);
    ...
    const handleSync = async () => {
        setSyncing(true);
        try {
            const res = await api.post("/bookmarks/sync");
            const { created, updated, total: t } = res.data.data;
            toast.success(`Sync selesai! ${created} baru, ${updated} diperbarui dari ${t} repo`);
            setPage(1);
            fetchBookmarks(true);
            fetchAllTopics();
        } catch {
            toast.error("Gagal sync bookmarks dari GitHub");
        } finally {
            setSyncing(false);
        }
    };
        ...
        <div className="space-y-5">
            ...
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Koleksi GitHub repos yang disimpan</p>
                </div>
                <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}>
                    <Button onClick={handleSync} disabled={syncing} size="sm"
                        className="gap-1.5 bg-gray-900 hover:bg-gray-800 dark:bg-white dark:hover:bg-gray-100 dark:text-gray-900 text-white">
                        {syncing ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Github className="w-3.5 h-3.5" />}
                        <span className="text-sm">{syncing ? "Syncing..." : "Sync GitHub"}</span>
                    </Button>
                </motion.div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.06 }}
                className="flex items-start gap-2.5 px-3.5 py-2.5 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-800">
                <RefreshCw className="w-3.5 h-3.5 text-amber-500 mt-0.5 shrink-0" />
                <p className="text-xs text-amber-700 dark:text-amber-400 leading-relaxed">
                    Bookmark diambil otomatis dari <strong>GitHub Starred Repos</strong> kamu. Klik <strong>Sync GitHub</strong> untuk memperbarui data terbaru.
                </p>
            </motion.div>
            ...
            {!initialLoading && total > 0 && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }}
                    ...
                    {[
                        { label: "Total Repo", value: total, color: "text-amber-500" },
            ...
            <AnimatePresence mode="wait">
                {initialLoading ? (
                    /* Skeleton — sama persis dengan blogs */
                    ...
                    <motion.div key="empty" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
                        ...
                        <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
                            {search || activeTopic ? "Coba kata kunci atau filter lain" : "Klik Sync GitHub untuk mengambil data"}
                        </p>
                        {!search && !activeTopic && (
                            <Button onClick={handleSync} disabled={syncing} size="sm"
                                className="mt-4 gap-1.5 bg-gray-900 hover:bg-gray-800 dark:bg-white dark:text-gray-900 text-white">
                                {syncing ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Github className="w-3.5 h-3.5" />}
                                Sync GitHub
                            </Button>
                        )}
                                    ...
                                    <motion.div key={bookmark.id} variants={itemVariants} exit="exit" layout
                                        ...
                                        <div className="flex items-start gap-2 sm:gap-3">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                {/* Badges row — tanda punya topics / description */}
                ...
                <AlertDialogContent className="bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 max-w-sm">
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription className="text-gray-500 dark:text-gray-400 text-sm">
                            Bookmark <strong className="text-gray-700 dark:text-gray-300">"{selectedBookmark?.title}"</strong> akan dihapus permanen. Kamu bisa sync ulang dari GitHub kapanpun.
